...
**Current state:** converts `score.abc` into a MIDI file and, together with the YuE audio, into a Logic Pro project — from the command line or in a web interface. The way back works as well: a MIDI file edited in Logic becomes a `score.abc` for YuE2 again (see [Back to YuE2](#back-to-yue2-midi-to-abc)).
...
| `POST /api/midi/abc` | multipart form: `file` (a MIDI file, up to 4 MB), optional `options` (JSON: `{"trackRoles": {"1": "Ignore"}, "skipBars": 0}`) | `200` with `abc`, the score, the file's `tracks` (index, name, channel, notes, role) and diagnostics as JSON; `422` if no score could be made, with the tracks still listed; `400` for a missing file or malformed options. See [Back to YuE2](#back-to-yue2-midi-to-abc) |
| `GET /api/health` | – | `ok` |
...
**Vocals of your own as a WAV.** Instead of the separated vocals, *Change the voice* also takes a WAV file of your own (*WAV file of your own*) — then no separation has to run first, and without a stem service configured this is the only way. The server checks the RIFF header to make sure it really is a WAV and hands it to the service under its name (200 MB at most — ChangeMyVoice takes no more, and no more than seven minutes either). The result goes into the next Logic project like any other once a `score.abc` is loaded; without a score there is no project, and the window at the end offers the download instead, named after the file (`<name>-voice.wav`). Unlike the separated vocals, a WAV of your own is not tied to the `audio.flac`: another recording leaves its job alone, another WAV starts over.

**The collection of model voices.** A model voice is one recording of a voice, kept by the service under a name — the collection a conversion chooses its timbre from, as the instrument library is what tracks are routed with. The dialog *Model voices* (the microphone in the header, shown only when a voice service is configured) lists what the service has, with the properties of the recording as it was uploaded and as the service keeps it, and adds one from a name and a file (WAV, MP3, FLAC, M4A/AAC or OGG/Opus). The service keeps the first 25 seconds as mono PCM at 44.1 kHz, since that is all the model uses; clean, dry singing without accompaniment gives the best result. *Download* fetches a voice as the service keeps it — exactly what the model gets to hear, so the right thing to check whether a voice is any good before a job computes on it for minutes; the uploaded original is not kept by the service. This needs a ChangeMyVoice with the route `GET /api/v1/voices/{id}/audio`; an older one answers `404` without the code `VOICE_NOT_FOUND`, which this server passes on as `501` and the window explains. A voice that a job is still waiting for cannot be removed, which the service refuses with `409`.

**The jobs.** The second microphone icon opens the voice service's jobs: what it knows, refreshed every five seconds while the window is open, this browser tab's job marked, and the buttons per job — *Cancel* for one that is waiting or running, *Download* and *Remove* for a finished one whose result the service still holds. *Download* fetches the converted vocals as a WAV and changes nothing at the service, which is also how one gets at the result of a job another browser or another tab started and that the *Change the voice* section therefore does not know about. *Remove* throws the result away afterwards. A job removed there that this tab was waiting for is let go of here as well. A job whose files the service has cleared away — on request or after its retention — stays on as a record: its row then says *cleared* instead of offering a button, so a removal that worked is not mistaken for one that did nothing. The list therefore only grows: it comes twenty-five at a time, *Back* and *Next* page through it, the line beside them says which part of how many is shown, and the select narrows it to one state — which is how one looks for what is waiting or what failed. Not every ChangeMyVoice offers the route for all of its jobs; one that does not answers `404`, which this server passes on as `501` and the window explains, instead of showing an empty list.
...
| `GET /api/voice/voices/{id}/audio` | – | the recording of the voice as the service keeps it, as a WAV; `501` from a service that cannot hand it out |
| `DELETE /api/voice/voices/{id}` | – | `204`; `409` while a job still waits for the voice |
| `POST /api/voice/jobs` | form with `voiceId` and either `stemJob` (the id of a finished separation) or `file` (a WAV of vocals) | `202` with the job; `400` without a voice, without a source, with both sources or with a file that is not a WAV; `501` for `stemJob` without a stem service |
...
| `GET /api/voice/jobs?status=&limit=&offset=` | – | one page of the jobs, newest first: `{ "jobs": [...], "total", "limit", "offset" }`, each job with `hasResult` saying whether anything is left to fetch. `limit` is 1 to 200 (the service takes 50 without one), `status` narrows to `QUEUED`, `RUNNING`, `COMPLETED`, `FAILED` or `CANCELLED`; `501` from a service that cannot list them |

The converted vocals need not travel through the browser either: the Logic export takes a field `voiceJob` with the job id, and the server fetches the WAV itself and puts it on the project's vocals track — in place of the separated vocals, while the dry ones, where a separation made them, keep their own track. Afterwards it confirms the import, whereupon the service drops the result. Two things are checked before the file goes into the package, and each of them leaves the converted vocals out of the project — the vocals track then keeps the separated ones, where there are any — with a warning (`YTL056`) rather than failing an export that is otherwise fine: the checksum the service names for its result, so that a transfer which broke off does not end up in the project as a truncated file, and the sample rate, since the project's audio tracks are prepared for 48 kHz while ChangeMyVoice works at its model's own; it delivers its result at 48 kHz, but that is not taken for granted. In both cases the result stays at the service, and so does the job in the interface: the *Change the voice* section keeps its download, says that the vocals did not go into the project, and points at the warnings of the export for the reason. The WAV can then be dropped onto the project's vocals track in Logic by hand.
...
## Back to YuE2: MIDI to ABC

The way back, for when a song has been generated, opened in Logic and changed there: a MIDI file becomes a `score.abc` again, which YuE2 can carry on with. In Logic, select all regions and export them with *File → Export → Selection as MIDI File*; then drop the file on the card *Back to YuE2* in the web interface and copy the score (or download it as `.abc`), or on the command line:

```sh
dotnet run --project src/YueToLogic.Cli -- song.mid     # writes song.abc
```

| Option | Meaning |
|---|---|
| `-o, --output <file>` | Score to write; `.abc` is appended if missing (default: `<input>.abc`) |
| `--track <n>=<role>` | Role of the `n`-th track as the summary lists it: `vocal`, `ins`, `chords` or `ignore`; repeatable |
| `--skip-bars <n>` | Leave out the first `n` bars (default: the silent ones before the first note) |
| `--dump-json <file>`, `-f`, `-v` | as above |

What becomes of the file undoes the way there, as far as the dialect can say it:

- **Tracks.** Each track (each channel of a track that plays on several) gets a role by its name: `Vocal`, `Ins` and `Chords`, as the MIDI file and the Logic project of this tool name them — Logic's *Bass · Mother32* counts by the part before the dot. Bass, drums, guide tones, doublings and channel 10 are left out. A file without those names gets them by sound and order: a track that strikes chords becomes Chords, the first other one Vocal, the next Ins. The web interface lists the tracks with a selection for each, `--track` does the same (`YTL076` reports what became what).
- **Rhythm.** Notes move to the nearest sixteenth, the unit of YuE2's scores; that also undoes swing and humanizing, while triplets cannot be written (`YTL072` counts the moved notes). Each voice plays one note at a time: of notes starting together the highest stays, a note ends where the next begins (`YTL073`).
- **Chords.** The chord track is read back into symbols of YuE2's vocabulary, beat by beat: block chords, the pulses, the off-beat stabs and the arpeggios the arranger plays, or what was played in Logic, slash chords included (C/E is a bass set apart below the chord, E G C an inversion). Some sets name two chords at once — C E G A is C6 over C and Am7 over A — and come back as the one the bass suggests; the same notes sound. Beats that form no chord keep the one before (`YTL074`). As in YuE2's scores, the chord in effect is written at the start of every bar of `Vocal`.
- **Start and tempo.** Silent bars at the start, such as a [count-in](#count-in), are left out (`YTL075`); `--skip-bars` or the field in the web interface set the number by hand. YuE2 knows one tempo: the one at the start is kept, rounded to whole BPM (`YTL077`). Meter and key changes go to the bar line (`YTL078`), markers become section comments (`% chorus`).

The score is laid out as YuE2 writes its own: `L:1/16`, groups of four bars with a line per voice, `z16` for an empty bar of `Vocal` and `Z4` for an empty group of `Ins`. The official example comes back character for character, from its MIDI file with bass, drums and a count-in as well. A file that is no MIDI file is refused (`YTL070`), one whose tracks give no notes for any voice as well (`YTL071`), but its tracks are still listed so their roles can be chosen.
...
- `services.AddYueToLogic()` registers the stateless `IScoreConverter`, `IAbcScoreParser`, `IScoreArranger`, `IMidiRenderer`, `IAbcScoreWriter` and `IMidiToAbcConverter` for dependency injection.
