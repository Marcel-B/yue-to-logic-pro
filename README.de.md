...
**Eigener Gesang als WAV.** Statt des getrennten Gesangs lässt sich unter *Stimme ändern* auch eine eigene WAV-Datei wählen (*Eigene WAV-Datei*) – dann muss vorher keine Trennung laufen, und ohne eingerichteten Stem-Dienst ist das der einzige Weg. Der Server prüft am RIFF-Kopf, dass es wirklich eine WAV ist, und reicht sie unter ihrem Namen an den Dienst weiter (höchstens 200 MB – mehr nimmt ChangeMyVoice nicht an, und auch nicht mehr als sieben Minuten). Das Ergebnis geht wie jedes andere mit ins nächste Logic-Projekt, sobald eine `score.abc` geladen ist; ohne Score gibt es kein Projekt, und das Fenster am Ende bietet stattdessen gleich den Download an, benannt nach der Datei (`<name>-voice.wav`). Anders als der getrennte Gesang hängt die eigene WAV nicht an der `audio.flac`: Eine andere Aufnahme lässt ihren Auftrag stehen, eine andere WAV beginnt von vorn.

**Die Sammlung der Modellstimmen.** Eine Modellstimme ist eine Aufnahme einer Stimme, vom Dienst unter einem Namen abgelegt – die Sammlung, aus der eine Umwandlung ihr Timbre wählt, so wie die Instrumentenbibliothek das ist, worauf Spuren geroutet werden. Das Fenster *Modellstimmen* (das Mikrofon in der Kopfzeile, nur mit eingerichtetem Voice-Dienst zu sehen) listet, was der Dienst hat, samt den Eigenschaften der Aufnahme, wie sie hochgeladen wurde und wie der Dienst sie behält, und legt aus Name und Datei eine neue an (WAV, MP3, FLAC, M4A/AAC oder OGG/Opus). Der Dienst behält davon die ersten 25 Sekunden als Mono-PCM mit 44,1 kHz, mehr braucht das Modell nicht; sauberer, trockener Gesang ohne Begleitung führt zum besten Ergebnis. *Laden* holt eine Stimme so zurück, wie der Dienst sie behält – genau das, was das Modell zu hören bekommt, also das Richtige, um nachzuprüfen, ob eine Stimme taugt, bevor ein Auftrag minutenlang auf ihr rechnet; das hochgeladene Original hebt der Dienst nicht auf. Das braucht ein ChangeMyVoice mit der Route `GET /api/v1/voices/{id}/audio`; ein älteres antwortet mit `404` ohne den Code `VOICE_NOT_FOUND`, was dieser Server als `501` weitergibt und das Fenster erklärt. Eine Stimme, auf die noch ein Auftrag wartet, lässt sich nicht löschen – der Dienst lehnt das mit `409` ab.

**Die Aufträge.** Das zweite Mikrofon-Symbol öffnet die Aufträge des Voice-Dienstes: was er kennt, alle fünf Sekunden aktualisiert, solange das Fenster offen ist, der Auftrag dieses Browser-Tabs markiert, und je Auftrag die Knöpfe – *Abbrechen* für einen wartenden oder laufenden, *Laden* und *Löschen* für einen fertigen, dessen Ergebnis der Dienst noch hat. *Laden* holt den geänderten Gesang als WAV, ohne beim Dienst etwas zu ändern: So kommt man auch an das Ergebnis eines Auftrags, den ein anderer Browser oder ein anderer Tab angelegt hat und den der Abschnitt *Stimme ändern* deshalb nicht kennt. *Löschen* wirft das Ergebnis danach weg. Wird dort der Auftrag gelöscht, auf den dieser Tab wartet, lässt auch er ihn los. Ein Auftrag, dessen Dateien der Dienst aufgeräumt hat – auf Wunsch oder nach Ablauf der Frist –, bleibt als Eintrag erhalten: In seiner Zeile steht dann *aufgeräumt* statt eines Knopfes, damit ein geglücktes Löschen nicht wie eines aussieht, das nichts bewirkt hat. Die Liste wächst also immer weiter: Sie kommt in Seiten zu fünfundzwanzig, *Zurück* und *Weiter* blättern, die Zeile daneben sagt, welcher Teil von wie vielen zu sehen ist, und die Auswahl schränkt auf einen Zustand ein – so findet man, was wartet oder fehlgeschlagen ist. Nicht jedes ChangeMyVoice kennt die Route für alle seine Aufträge; eines ohne sie antwortet mit `404`, was dieser Server als `501` weiterreicht und das Fenster erklärt, statt eine leere Liste zu zeigen.
...
| `GET /api/voice/voices/{id}/audio` | – | die Aufnahme der Stimme, wie der Dienst sie behält, als WAV; `501` von einem Dienst, der sie nicht herausgeben kann |
| `DELETE /api/voice/voices/{id}` | – | `204`; `409`, solange noch ein Auftrag auf die Stimme wartet |
| `POST /api/voice/jobs` | Formular mit `voiceId` und entweder `stemJob` (die ID einer fertigen Trennung) oder `file` (eine WAV mit Gesang) | `202` mit dem Auftrag; `400` ohne Stimme, ohne Quelle, mit beiden Quellen oder mit einer Datei, die keine WAV ist; `501` für `stemJob` ohne Stem-Dienst |
...
| `GET /api/voice/jobs?status=&limit=&offset=` | – | eine Seite der Aufträge, jüngste zuerst: `{ "jobs": [...], "total", "limit", "offset" }`, je Auftrag mit `hasResult` für die Frage, ob noch etwas abzuholen ist. `limit` reicht von 1 bis 200 (ohne Angabe nimmt der Dienst 50), `status` schränkt auf `QUEUED`, `RUNNING`, `COMPLETED`, `FAILED` oder `CANCELLED` ein; `501` von einem Dienst, der sie nicht auflisten kann |

Auch der geänderte Gesang muss nicht durch den Browser: Beim Logic-Export genügt das Feld `voiceJob` mit der Auftrags-ID, dann holt der Server die WAV selbst und legt sie auf die Vocals-Spur des Projekts – anstelle des getrennten Gesangs, während der trockene, wo eine Trennung ihn erzeugt hat, seine eigene Spur behält. Danach bestätigt er den Import, womit der Dienst das Ergebnis löscht. Zwei Dinge werden geprüft, bevor die Datei ins Paket wandert, und jedes von beiden lässt den geänderten Gesang aus dem Projekt – die Vocals-Spur behält dann den getrennten, wo es einen gibt – und meldet eine Warnung (`YTL056`), statt einen sonst fehlerfreien Export scheitern zu lassen: die Prüfsumme, die der Dienst für sein Ergebnis nennt, damit ein abgebrochener Transfer nicht als abgeschnittene Datei im Projekt landet, und die Abtastrate, denn die Audiospuren des Projekts sind auf 48 kHz vorbereitet, während ChangeMyVoice mit der Rate seines Modells rechnet; es liefert sein Ergebnis zwar in 48 kHz aus, aber das wird nicht vorausgesetzt. In beiden Fällen bleibt das Ergebnis beim Dienst – und der Auftrag in der Oberfläche: Der Abschnitt *Stimme ändern* behält seinen Download, sagt, dass der Gesang nicht ins Projekt kam, und verweist für den Grund auf die Hinweise des Exports. Die WAV lässt sich dann in Logic von Hand auf die Vocals-Spur ziehen.
...
src/YueToLogic.Api/     ASP.NET-Core-API; liefert das Web-Frontend unter /ui aus
  ClientApp/            Vue-3-Frontend mit Vite und TypeScript (PrimeVue, Tailwind)
