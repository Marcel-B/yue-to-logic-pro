...
/** PrimeVue's Textarea renders the element itself as its root, so $el is the textarea. */
const text = useTemplateRef<{ $el: HTMLTextAreaElement }>('text')
...
const roleOptions = computed(() => ROLES.map((role) => ({ label: roleLabel(role), value: role })))

/**
 * InputNumber reports its value while typing and again on blur, and has no change event of its own; asking the
 * server only when the number really changed keeps that to one request per edit (a newer one aborts the older).
 */
function setSkipBars(value: number | null): void {
  const next = value ?? ''
  if (next === skipBars.value) {
    return
  }
  skipBars.value = next
  void convert()
...
<template>
  <Card aria-live="polite">
    <template #title>
      <h2 class="m-0">{{ t('midiTitle') }}</h2>
    </template>
    <template #content>
      <p class="muted mt-0 mb-4 text-sm">{{ t('midiInfo') }}</p>
      <FileDropZone
        :file="file"
        :extension="['.mid', '.midi']"
      ...
      <template v-if="result">
        ...
        <template v-if="result.tracks.length">
          <h3>{{ t('midiTracks') }}</h3>
          <p class="hint muted mt-0">{{ t('midiTracksHint') }}</p>
          <ul class="tracks">
            <li v-for="track in result.tracks" :key="track.index">
              ...
              <span class="muted text-sm">
                {{ t('midiTrackChannel', { channel: track.channel }) }} · {{ t('notes', { count: track.noteCount }) }}
              </span>
              <Select
                :model-value="track.role"
                :options="roleOptions"
                option-label="label"
                option-value="value"
                :disabled="busy"
                ...
                size="small"
                fluid
                @update:model-value="assign(track.index, $event)"
              />
            </li>
          </ul>
        </template>

        <div class="mt-5 flex max-w-64 flex-col gap-1">
          <label for="midi-skip-bars" class="text-sm text-muted-color">{{ t('midiSkipBars') }}</label>
          <InputNumber
            :model-value="skipBars === '' ? null : skipBars"
            input-id="midi-skip-bars"
            :min="0"
            :max="999"
            :step="1"
            :use-grouping="false"
            ...
            :disabled="busy"
            fluid
            @update:model-value="setSkipBars"
          />
        </div>
        ...
        <template v-if="result.abc">
          ...
          <Textarea
            ref="text"
            class="abc"
            :model-value="result.abc"
            readonly
            spellcheck="false"
            rows="14"
            fluid
            @focus="text?.$el.select()"
          />
          <div class="mt-3 flex flex-wrap gap-2">
            <Button
              :icon="copied ? 'pi pi-check' : 'pi pi-copy'"
              :label="copied ? t('midiCopied') : t('midiCopy')"
              @click="copy"
            />
            <Button
              severity="secondary"
...
/* Name, facts and role side by side where there is room; stacked on a phone. */
.tracks li {
  display: grid;
...
/* The score reads as code: monospaced, small, on the sunken surface like the other code blocks. */
.abc {
  margin-top: 0.5rem;
