...
public sealed class MidiRenderer : IMidiRenderer
{
    ...
    private static TrackChunk BuildChordTrack(IReadOnlyList<ChordEvent> chords, FourBitNumber channel, int velocity)
    {
    ...
    private static FourBitNumber NextChannel(ref int channel)
    {
        if (channel == GeneralMidiDrums.Channel)
        {
            channel++;
        }

        var result = (FourBitNumber)(byte)Math.Min(channel, 15);
        channel++;
        return result;
    }
